...
resource "local_file" "logstash_metricbeat_pipeline" {
  content = templatefile(
    "${var.working_dir}/${var.logstash_metricbeat_pipeline}", 
    {
      elasticsearch_ip = aws_instance.elasticsearch.private_ip,
    }
  )
  filename = "${var.working_dir}/${var.logstash_metricbeat_pipeline_rendered}"
}
